...
static const int turningTime = 750	; // hardcoded
...
int main(void)
{
	...
	while(1) {
		...
		do {
			...
			if(speed > 50) delayDeadEnd = 125;
			else delayDeadEnd = 150;
			...
			// ficarem a 0000 nao comece logo a inverter o sentido
			if(deadEnd() && !isTurning) {
			...
			} else if((((sideDetected == 1) || (sideDetected == 3)) && (sensor == 0x02) && isTurning) ||
				   ((sideDetected == 2) && (sensor == 0x08) && isTurning) || 
			           ((sideDetected == 4) && (sensor == 0x08) && (millis >= turningTime) && isTurning) ||
